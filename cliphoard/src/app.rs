...
impl Default for SettingsLimits {
    fn default() -> Self {
        Self {
            max_unpinned: cliphoard_schema::DEFAULT_MAX_UNPINNED.to_string(),
            max_pinned: cliphoard_schema::DEFAULT_MAX_PINNED.to_string(),
            max_entry_size: (cliphoard_schema::DEFAULT_MAX_ENTRY_SIZE / (1024 * 1024)).to_string(),
        }
    }
}
...
impl OverlayState {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::SelectIndex(idx) => {
                ...
                let offset = AbsoluteOffset { x: 0.0, y: idx as f32 * ITEM_HEIGHT };
            ...
            Message::SelectNext => {
                ...
                if filtered_count > 0 {
                    ...
                    let offset = AbsoluteOffset { x: 0.0, y: new_index as f32 * ITEM_HEIGHT };
                    let unfocus = widget::text_input::focus(widget::Id::unique());
                    return Task::batch(vec![unfocus, scroll_to(self.scrollable_id.clone(), offset)]);
                }
            }
            ...
            Message::SettingsKeybindingChanged(name, value) => {
                match name.as_str() {
                    "toggle_pin" => {
            ...
            Message::KeybindingCaptured(keybinding_str) => {
                ...
                match which {
                    CapturingKeybinding::TogglePin => {
                        ...
                        self.keybinding_errors.toggle_pin = KeyBinding::from_str(&keybinding_str).err();
                    }
                    CapturingKeybinding::TogglePreview => {
                        self.settings_keybindings.toggle_preview = keybinding_str.clone();
                        self.keybinding_errors.toggle_preview = KeyBinding::from_str(&keybinding_str).err();
                    }
                    CapturingKeybinding::DeleteEntry => {
                        self.settings_keybindings.delete_entry = keybinding_str.clone();
                        self.keybinding_errors.delete_entry = KeyBinding::from_str(&keybinding_str).err();
                    }
                    CapturingKeybinding::TabAll => {
                        self.settings_keybindings.tab_all = keybinding_str.clone();
                        self.keybinding_errors.tab_all = KeyBinding::from_str(&keybinding_str).err();
                    }
                    CapturingKeybinding::TabPinned => {
                        self.settings_keybindings.tab_pinned = keybinding_str.clone();
                        self.keybinding_errors.tab_pinned = KeyBinding::from_str(&keybinding_str).err();
                    }
            ...
            Message::SaveSettings => {
                ...
                let max_entry_size = self.settings_limits.max_entry_size.parse().unwrap_or(10) * 1024 * 1024;
    ...
    fn view(&self, _id: window::Id) -> Element<'_, Message> {
            ...
            for (idx, entry) in filtered.iter().enumerate() {
                ...
                let row_container = widget::container(
                        widget::mouse_area(row)
                            .on_press(Message::SelectIndex(idx)),
                    )
                    .width(Length::Fill)
                    ...
                    if let Some(entry) = filtered.get(idx) {
                        if entry.mime.is_text() {
                            ...
                            let image_widget: Element<'_, Message> =
                                if entry.mime == cliphoard_schema::MimeType::ImageSvg {
        ...
        let panel = widget::container(main_col)
            ...
            .class(cosmic::theme::Container::Custom(Box::new(|theme: &Theme| {
                let cosmic = theme.cosmic();
