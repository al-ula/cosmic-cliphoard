...
//! D-Bus service implementation for the clipboard manager.

use cliphoard_schema::{
    ClipboardConfig, ClipboardHistory, OxiCodeCodec, Codec,
    DBUS_NAME, DBUS_PATH, EntryId, OxiCodeError, PinResult,
};
use std::sync::Arc;
...
impl ClipboardManagerService {
    pub fn new(history: Arc<RwLock<ClipboardHistory>>, storage: Arc<crate::storage::Storage>) -> Self {
...
impl ClipboardManagerService {
    async fn list_entries(&self) -> zbus::fdo::Result<Vec<u8>> {
        ...
        OxiCodeCodec::serialize(&entries)
    ...
    async fn get_entry(&self, id: u64) -> zbus::fdo::Result<Vec<u8>> {
        ...
        match hist.get(EntryId(id)) {
            Some(entry) => OxiCodeCodec::serialize(entry)
                .map_err(|e| zbus::fdo::Error::Failed(e.to_string())),
    ...
    async fn search(&self, query: &str) -> zbus::fdo::Result<Vec<u8>> {
        ...
        OxiCodeCodec::serialize(&results)
            .map_err(|e| zbus::fdo::Error::Failed(e.to_string()))
    }
    ...
    async fn update_config(
        ...
        debug!(max_unpinned, max_pinned, max_entry_size, "update_config called");
...
pub async fn run_daemon() -> Result<(), ServiceError> {
    ...
    history_data.update_limits(config.max_unpinned, config.max_pinned, config.max_entry_size);
    let history = Arc::new(RwLock::new(history_data));

    let watcher = crate::watcher::ClipboardWatcher::new(Arc::clone(&history));
    watcher.start().await?;
